(function($) {
    $.fn.emojify = function() {
        ...
        return this.each( function() {
            ...
            $( this ).html( $( this ).html().replace( regex, $.fn.emojify.replace ) );
        });
    };

    $.fn.emojify.replace = function(){
        var key = arguments[1];
        ...
        var srcPrefix = 'https://raw.githubusercontent.com/iamcal/emoji-data/master/';
        var emojionePrefix = 'https://raw.githubusercontent.com/Ranks/emojione/master/'; // This one is different

        var emojiJson = JSON.parse( localStorage.getItem( 'emojiJson' ) );
        ...
        if ( ! emojiJson ) {
            ...
            $.ajax({
                ...
                success: function ( data ) {
                    ...
                    emojiJson = JSON.stringify( data );

                    localStorage.setItem( 'emojiJson', emojiJson );

                    for ( var index = 0; index < data.length; index ++ ) {

                        if ( data[index].short_names.indexOf( key ) !== -1 ) {
                            apple = data[index].apple_img_path;
                            google = ( data[index].google_img_path === undefined ? apple : data[index].google_img_path );
                            twitter = ( data[index].twitter_img_path === undefined ? google : data[index].twitter_img_path );
                            emojione = ( data[index].emojione_img_path === undefined ? google : data[index].emojione_img_path );
                            emojione = emojione.replace( 'emojione/', '' );
                        }
            ...
            for ( var index = 0; index < emojiJson.length; index ++ ) {

                if ( emojiJson[index].short_names.indexOf( key ) !== -1 ) {
                    apple = emojiJson[index].apple_img_path;
                    google = ( emojiJson[index].google_img_path === undefined ? apple : emojiJson[index].google_img_path );
                    twitter = ( emojiJson[index].twitter_img_path === undefined ? google : emojiJson[index].twitter_img_path );
                    emojione = ( emojiJson[index].emojione_img_path === undefined ? google : emojiJson[index].emojione_img_path );
                    emojione = emojione.replace( 'emojione/', '' );
                }
        ...
        return '<img class="emoji" width="20" height="20" align="absmiddle" src="' + srcPrefix + apple + '" alt="' + key + '" title="' + key + '" /><img class="emoji" width="20" height="20" align="absmiddle" src="' + srcPrefix + google + '" alt="' + key + '" title="' + key + '" /><img class="emoji" width="20" height="20" align="absmiddle" src="' + srcPrefix + twitter + '" alt="' + key + '" title="' + key + '" /><img class="emoji" width="20" height="20" align="absmiddle" src="' + emojionePrefix + emojione + '" alt="' + key + '" title="' + key + '" />';
